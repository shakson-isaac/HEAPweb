...
def verify(src, ledger):
    """Every file the ledger claims must exist with the right hash, and the
    tree must contain nothing the ledger does not know about."""
    if not os.path.exists(ledger):
        sys.exit(f"missing ledger {ledger} -- run build_payload.py first")
    expect = {}
...
def main():
    ...
    ap.add_argument("--ledger", default=os.path.join(repo, "build", "web", "manifest.tsv"))
    ...
    n, total = verify(args.src, args.ledger)
