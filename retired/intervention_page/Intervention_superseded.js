// ============================================================================
// RETIRED -- NOT RENDERED. Kept as the record of how these panels were built.
//
// These five sections were the Interventions page before it was rebuilt around
// InterventionConcordance (one annotation-controlled scatter) and
// InterventionNetwork (the Fig 5d network for any disease):
//
//   ScatterSection    UKB beta vs trial effect, one trial at a time
//   CompareSection    the exposure x trial correlation heatmap
//   ExemplarSection   per-exposure exemplar proteins
//   MrEdgeSection     MR edge counts as a grouped bar
//   ArmsSection       health-behaviour arms (a Module 2 panel whose own
//                     subtitle said no trial data enters it)
//
// WHY THEY WERE RETIRED, and it was not a matter of taste. Every one was fed by
// a FIGURE EXPORT rather than the supplementary deposit, so they carried 174
// exposure terms and no standard errors, where the deposit has 185 terms with
// se_HEAP on every row and the trial SEs besides. The heatmap additionally
// truncated to 65 of 97 exposure terms without saying so on screen
// (fig_intervention_compare.R applies a significance filter, collapses field
// levels, then caps at TOP_N = 28 labels).
//
// The root cause sat upstream: all five Fig 5/6 plotters carry
// website_export = no in HEAP/config/figures/figure_registry.tsv, so the page
// was fed by their SUPERSEDED predecessors and never saw the current figures.
//
// Nothing here is imported. It is not linted or bundled -- the file lives
// outside src/ deliberately, so it cannot drift back into the build by someone
// adding an import. Read it, do not revive it: the data it reads is thinner
// than what the page now has.
// ============================================================================
