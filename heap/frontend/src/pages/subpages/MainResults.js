...
// Variance-component palette, mirroring HEAP_PAL_COMPONENT in
// HEAP/scripts/visualizations/common/plot_theme.R. Exposure *categories* are a
// different axis and use ecatColor() (S6); these are model blocks.

// Section payloads label the same block several ways ("Genetic", "Genetic (PGS)",
// "C+G"), so match on the stem rather than adding a key per spelling.
function compColor(c) {
  const k = String(c);
  if (COMP_COLORS[k]) return COMP_COLORS[k];
  if (/^GxE/.test(k)) return COMP_COLORS.GxE;
  if (/^Genetic/.test(k)) return COMP_COLORS.Genetic;
  if (/^Exposom/.test(k)) return COMP_COLORS.Exposome;
  return COMP_COLORS.Covars;
}

const DRIVER_COLORS = {
  'Genetics-driven': COMP_COLORS.Genetic,
  'Exposures-driven': COMP_COLORS.Exposome,
};

// ---------------------------------------------------------------------------
// a. Genetics versus exposome, per protein
// ---------------------------------------------------------------------------
function GenoVsExpo() {
