...
//
// Five earlier panels were retired here rather than deleted; the code and the
// reasoning are in HEAPweb/retired/intervention_page/. They were fed by figure
// exports rather than the supplementary deposit, so they showed fewer exposures
// and no standard errors, and one of them silently truncated to 65 of 97
// exposure terms.
// ---------------------------------------------------------------------------
  ...
  return (
    <Box sx={{ mt: 3 }}>
      <Typography variant="body1" sx={{ mb: 2, maxWidth: 900 }}>
        HEAP&apos;s lifestyle signatures are observational associations measured in UK
        Biobank. This page places them next to protein effects reported by randomized
        interventions: HERITAGE, an exercise-training trial, and STEP 1 / STEP 2, trials
        of a GLP-1 receptor agonist. Everything below compares effect estimates from
        separate studies on separate assay platforms.
      </Typography>
      ...
      <Alert severity="warning" sx={{ mb: 2, maxWidth: 1000 }}>
        ...
        different quantities in different units. Read agreement in <i>sign and rank</i>,
        not in magnitude.
      </Alert>

      <Alert severity="info" sx={{ mb: 3, maxWidth: 1000 }}>
        <b>Both trial arms are significance-selected, which flatters the agreement.</b>{' '}
        ...
        q &lt; 0.05 before any join. So every correlation here is computed against trial
        effects pre-selected to be large, which biases it upward. That is a property of
        what the trials published, not of the estimates themselves &mdash; but it means
        these correlations are an upper bound on concordance rather than an unbiased
        estimate of it.
      </Alert>
